...
def calculate_sifid_given_paths(path1, path2, batch_size, cuda, dims, suffix):
    ...
    fid_values = []
    # Im_ind = []
    # if os.path.exists(f'{path2}/layer2_ind2fid.txt'):
    #     os.remove(f'{path2}/layer2_ind2fid.txt')
    # f = open(f'{path2}/layer2_ind2fid.txt', 'a')
    # max_ind2fid = {}
    for i in range(len(files2)):
        min_value = 10
        if os.path.isdir(files2[i]):
            m1, s1 = calculate_activation_statistics([files1[i]], model, batch_size, dims, cuda)
            for root, dirs, files in os.walk(files2[i]):
                # if os.path.relpath(root, files2[i]) == 'train_j8_layer2':
                    for file in files:
                        file_path = os.path.join(root, file)
                        m2, s2 = calculate_activation_statistics([file_path], model, batch_size, dims, cuda)
                        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
                        if fid_value < min_value:
                            min_value = fid_value
                            max_ind2fid = {file_path: fid_value}
            fid_values.append(min_value)
            # f.writelines(str(max_ind2fid) + '\n')
        else:
            m1, s1 = calculate_activation_statistics([files1[i]], model, batch_size, dims, cuda)
            ...
            # f.writelines(file_num1 + ',' + file_num2 + ':' + str(fid_value) + '\n')
            if fid_value < min_value:
                min_value = fid_value
                max_ind2fid = {file_num2: fid_value}
    # f.writelines(str(max_ind2fid) + '\n' + '\n')
    # f.close()
    return fid_values
...
if __name__ == '__main__':
    ...
    # with open('imagenet50_0.98_0_0.05_sifid.txt', 'a') as f:
    #     f.writelines(str(sifid_values.mean()) + '\n')
