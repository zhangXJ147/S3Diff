...
class ClipExtractor(torch.nn.Module):
    def __init__(self, pretrained=True, path=None, clip_affine_transform_fill=True, n_aug=16):
        global model
        super().__init__()

        ckpt = 'ViT-B/32' if path is None else path  # ViT-B/32, RN50

        if pretrained:
            model = clip.load(ckpt, device=device)[0]
