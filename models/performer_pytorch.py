...
class Performer(nn.Module):
    def __init__(
        ...
        if use_scalenorm:
            wrapper_fn = partial(PreScaleNorm, inner_dim)
        else:
            wrapper_fn = partial(PreLayerNorm, inner_dim)
        ...
        attn = SelfAttention(inner_dim, causal=causal, heads=heads, dim_head=dim_head,
                             local_heads=local_attn_heads, nb_features=nb_features,
        ...
        ff = Chunk(ff_chunks, FeedForward(inner_dim, mult=ff_mult, dropout=ff_dropout, glu=ff_glu), along_dim=1)

        if shift_tokens:
            shift = (0, 1) if causal else (-1, 0, 1)
            attn, ff = map(lambda t: PreShiftTokens(shift, t), (attn, ff))

        self.attn1, self.ff1 = map(wrapper_fn, (attn, ff))

        self.attn2 = wrapper_fn(CrossAttention(inner_dim, heads=heads, dim_head=dim_head, nb_features=nb_features,
                                               generalized_attention=generalized_attention, kernel_fn=kernel_fn,
                                               dropout=attn_dropout, no_projection=no_projection, qkv_bias=qkv_bias,
                                               attn_out_bias=attn_out_bias))
        self.ff2 = wrapper_fn(
            Chunk(ff_chunks, FeedForward(inner_dim, mult=ff_mult, dropout=ff_dropout, glu=ff_glu), along_dim=1))
    ...
    def forward(self, x, context, **kwargs):
        ...
        x = self.ff1(self.attn1(x, **kwargs))
        x = self.ff2(self.attn2(x, context=context, **kwargs))
